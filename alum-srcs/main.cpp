...
void FGE_PulsarTeclaCaracter( GLFWwindow* window, unsigned int codepoint )
{
   ...
   switch (toupper(ch))
   {
      ...
      case 'V' :
        {
        contextoVis.usarVBOs ^= true;
        string str_modo = contextoVis.usarVBOs ? "VBO" : "Inmediato";
        cout << "modo de visualización cambiado a: '" << str_modo << "'" << endl << flush ;
        }
        break ;
      case 'M' :
         contextoVis.modoVis = ModosVis((int(contextoVis.modoVis)+1) % (numModosVis-2)) ; // Poner -1 para activar modo examen
...
void FGE_PulsarTeclaEspecial( GLFWwindow* window, int key, int scancode, int action, int mods )
{
   ...
   // Comentamos estas 3 lineas de cara al examen
...
void Inicializa_OpenGL( )
{
  ...
  id_programa = CrearPrograma("simple_fs.glsl","simple_vs.glsl");
